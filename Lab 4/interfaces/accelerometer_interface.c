...
static MA_FILTER_TYPE x_buffer[MA_FILTER_DEPTH + 2];
static MA_FILTER_TYPE y_buffer[MA_FILTER_DEPTH + 2];
static MA_FILTER_TYPE z_buffer[MA_FILTER_DEPTH + 2];
...
//if LSM9DS1 is used for accelerometer readings, the normalization matrix would change
...
//if only the STMF32 board is being used for accelerometer readings
	static const float normalizing_matrix[3][3] = {
...
//if LSM9DS1 is attached, defined the configuration of it by setting data rate as 100Hz and enabling X, Y, and Z axes and enabled the interrupt for the driver
...
*	will reset semaphore to NULL once the operation is done for accelerometer sensor
*/
